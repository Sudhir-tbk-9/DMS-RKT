...
async function getFile(
  _: string,
  { arg }: { arg: { id?: string; name?: string; ids?: number[] } }
) {
  try {
    const params: { id?: string; name?: string; ids?: number[] } = {};
    if (arg.id) {
      params.id = arg.id;
    }
    if (arg.ids) {
    ...
    const data = await apiGetFolders<GetFoldersListResponse, typeof params>(
      params
    );
    ...
    if (!data || data.length === 0) {
      ...
      return {
        data: { content: [], page: { totalElements: 0, totalPages: 1 } },
      };
    }
    return data;
  } catch (error) {
...
const FolderManager = () => {
  ...
  const { trigger: triggerGetFile, isMutating: isFileMutating } =
    useSWRMutation(`/project-files`, getFile, {
      onSuccess: (resp) => {
        if (resp && resp.data) {
  ...
  useEffect(() => {
    if (openedDirectoryId) {
      triggerGetDoc({
        ...
        year: yearFilter,
        documentNumber: documentIdFilter,
      });
  ...
  const handleFolderEdit = (id: string) => {
    const folder = fileList.find(
      (f) => f.id === id && f.fileType === "directory"
    );
    ...
    try {
      ...
      if (fileExtension === "pdf") {
        ...
        console.log(
          ...
          fileExtension,
          "Using DocumentPreviewEnhanced as fallback"
        );
      }
    } catch (error) {
  ...
  const filteredFileList = useMemo(() => {
    ...
    // Apply file/folder name filter
    if (fileNameFilter) {
      currentList = currentList.filter((item) =>
        item.name.toLowerCase().includes(fileNameFilter.toLowerCase())
      );
    }

    // Apply document ID filter (only for files, or folders if they have a 'code' field)
    if (documentIdFilter) {
      currentList = currentList.filter((item) => {
        if (item.fileType !== "directory" && "documentNumber" in item) {
          return (item as File).documentNumber
            ?.toLowerCase()
            .includes(documentIdFilter.toLowerCase());
        } else if (item.fileType === "directory" && "code" in item) {
          return (item as Folder).code
            ?.toLowerCase()
            .includes(documentIdFilter.toLowerCase());
        }
        return true; // Keep if no relevant ID field or filter not applicable
      });
    }

    // Apply file type filter (only for files, and only if a directory is opened)
    if (FileTypeFilter && openedDirectoryId) {
      currentList = currentList.filter(
        (item) => item.fileType === FileTypeFilter
      );
    }

    // Apply category filter (only for files, and only if a directory is opened)
    if (categoryFilter && openedDirectoryId) {
      currentList = currentList.filter(
        (item) =>
          item.fileCategory?.toLowerCase() === categoryFilter.toLowerCase()
      );
    }

    // Apply year filter (only for files, and only if a directory is opened)
    if (yearFilter && openedDirectoryId) {
      currentList = currentList.filter((item) => {
        const uploadDate = (item as any).uploadDate; // Ensure uploadDate is accessed correctly
        if (uploadDate) {
          const uploadYear = new Date(uploadDate).getFullYear().toString();
          return uploadYear === yearFilter;
        }
        return false;
      });
    }
          ...
          {isFileMutating || isDocMutating ? (
              ...
              <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 mt-4 gap-4 lg:gap-6">
                {[...Array(4).keys()].map((item) => (
                  <FileSegment
                    key={item}
                    id={`loading-${item}`}
                    loading={isFileMutating || isDocMutating}
                  />
                ))}
              </div>
            ) : (
            ...
            <>
              <FileList
                ...
                totalItems={
                  openedDirectoryId ? fileTotalItems : folderTotalItems
                }
                totalPages={
                  openedDirectoryId ? fileTotalPages : folderTotalPages
                }
                ...
                onPageSizeChange={
                  openedDirectoryId
                    ? handleFilePageSizeChange
                    : handleFolderPageSizeChange
                }
                onPaginationChange={
                  openedDirectoryId
                    ? handleFilePaginationChange
                    : handleFolderPaginationChange
                }
              />
            </>
          )}
