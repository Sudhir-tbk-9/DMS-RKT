...
type PagersProps = {
    pageCount: number;
    currentPage: number; // API se aane wala page (1-based indexing)
    pagerClass: {
        default: string;
        inactive: string;
        active: string;
    ...
    onChange: (page: number) => void; // 1-based index bhejne ke liye ensure karein
};

const Pagers = ({ pageCount, currentPage, onChange, pagerClass }: PagersProps) => {
    const [showPrevMore, setShowPrevMore] = useState(false);
    const [showNextMore, setShowNextMore] = useState(false);
    ...
    const onPagerClick = (value: number, e: React.MouseEvent<HTMLLIElement>) => {
        e.preventDefault();
        if (value !== currentPage) {
            onChange(value); // API expects 1-based index
        }
    };
    ...
    const onArrowClick = useCallback(
        (e: More) => {
            let newPage = currentPage;
            if (e === 'nextMore') newPage = Math.min(currentPage + 5, pageCount);
            if (e === 'prevMore') newPage = Math.max(currentPage - 5, 1);
            onChange(newPage);
        },
    ...
    const getPages = useMemo(() => {
        ...
        if (showPrevMore && !showNextMore) {
            for (let i = pageCount - (PAGER_COUNT - 2); i < pageCount; i++) {
                pagerArray.push(i);
            }
        } else if (!showPrevMore && showNextMore) {
            for (let i = 2; i < PAGER_COUNT; i++) {
    ...
    const getPagerClass = (index: number) =>
        classNames(pagerClass.default, currentPage === index ? pagerClass.active : pagerClass.inactive);
    ...
    return (
        <ul>
            ...
            {showPrevMore && <PrevMore className={classNames(pagerClass.default, pagerClass.inactive)} onArrow={onArrowClick} />}
            {getPages.map((pager, index) => (
                <li key={index} className={getPagerClass(pager)} role="presentation" onClick={(e) => onPagerClick(pager, e)}>
                    {pager}
                </li>
            ))}
            {showNextMore && <NextMore className={classNames(pagerClass.default, pagerClass.inactive)} onArrow={onArrowClick} />}
            {pageCount > 1 && (
