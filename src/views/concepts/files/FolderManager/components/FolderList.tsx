...
import type { Folder } from "../types" // Import Folder type
...
const FileList = (props: FileListProps) => {
  ...
  // FIX: Derive folders from the fileList prop, directly using 'code'
  const folders = useMemo(() => {
    return fileList.filter((file) => file.fileType === "directory") as Folder[]
  }, [fileList])
    ...
    <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 mt-4 gap-4 lg:gap-6">
      {list.map((file) => (
        <FileSegment
          key={file.id}
          id={file.id}
          // Correctly pass directoryCode for folders and documentNumber for files
          directoryCode={isFolder ? (file as Folder).code : undefined} // Removed || "N/A"
          documentNumber={!isFolder ? (file as any).documentNumber : undefined}
